...
export class AuthController {
  ...
  private setAvatarConfirmCookie(res: FastifyReply): void {
    const isProd = this.configService.get<string>("NODE_ENV") === "production";
    res.setCookie("oauth_avatar_confirm", "1", {
      httpOnly: false,
      secure: isProd,
      sameSite: "lax",
      maxAge: 600,
  ...
  private async parseRegisterMultipart(
    req: FastifyRequest,
  ): Promise<{ dto: RegisterDto; avatarBuffer?: Buffer }> {
    ...
    for await (const part of parts) {
      if (part.type === "file") {
        if (part.fieldname === "avatar") {
          ...
          if (buffer.length > 0) avatarBuffer = buffer;
        } else {
          await part.toBuffer();
        }
      } else {
  ...
  private remainingAccessTokenTtl(token: string): number {
    try {
      ...
      // An unreadable token falls back to the default lifetime below.
    }
    return 900;
  }
  ...
  async token(@Body() dto: OAuthTokenDto) {
    ...
    switch (dto.grant_type) {
      case GrantType.PASSWORD: {
        ...
        return this.authService.generateTokens(user.id, { apiClient: true });
      }
  ...
  async register(@Req() req: FastifyRequest) {
    if (req.isMultipart()) {
      const { dto, avatarBuffer } = await this.parseRegisterMultipart(req);
      await this.authService.register(dto, avatarBuffer);
    } else {
  ...
  private async handleOAuthRedirect(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
      const tokens = await this.authService.generateTokens(user.id);
      this.setCookies(res, tokens);
      if (this.authService.consumeNeedsAvatarConfirm(user.id)) {
        this.setAvatarConfirmCookie(res);
      }

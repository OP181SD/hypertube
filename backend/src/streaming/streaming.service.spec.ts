...
describe("StreamingService", () => {
  ...
  describe("initiateStream", () => {
    it("should throw NotFoundException for unknown torrent", async () => {
      ...
      await expect(
        service.initiateStream("nonexistent", "user-1"),
  ...
  describe("getStreamStatus", () => {
    it("should throw NotFoundException for unknown torrent", async () => {
      ...
      await expect(service.getStreamStatus("nonexistent")).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("getVideoStream", () => {
    it("should throw NotFoundException when torrent not found", async () => {
      ...
      await expect(service.getVideoStream("nonexistent")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should throw NotFoundException when file not available", async () => {
      ...
      await expect(service.getVideoStream("t1")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should return transcoded stream for MKV files", async () => {
      ...
      expect(result.mimeType).toBe("video/mp4");
      expect(result.fileSize).toBeNull(); // Unknown for transcoded
    });

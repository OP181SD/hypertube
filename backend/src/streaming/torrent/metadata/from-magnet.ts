...
import { DEFAULT_TRACKERS, hexToInfoHash, infoHashToHex } from "./torrent-metadata";
...
export function parseMagnetUri(uri: string): MagnetLink {
    ...
    const trackers = params.getAll("tr").filter(Boolean);
    if (trackers.length === 0)
        DEFAULT_TRACKERS.forEach((t) => trackers.push(t));
    return {
        infoHash,

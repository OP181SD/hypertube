...
export function encodeCancel(pieceIndex: number, begin: number, length: number): Buffer {
    ...
    payload.writeUInt32BE(begin, 4);
    payload.writeUInt32BE(length, 8);
    return encodeMessage(MessageId.Cancel, payload);
}
...
export function encodePiece(pieceIndex: number, begin: number, block: Buffer): Buffer {
    ...
    return encodeMessage(MessageId.Piece, payload);
}
...
export function encodeBitfield(have: boolean[]): Buffer {
    const bytes = Math.ceil(have.length / 8);
    const bf = Buffer.alloc(bytes);
    for (let i = 0; i < have.length; i++) {
        if (!have[i])
            continue;
        bf[i >> 3]! |= 1 << (7 - (i & 7));
    }
    return encodeMessage(MessageId.Bitfield, bf);
}

...
export class MovieQueryService {
  ...
  buildWhereClause(params: SearchParams): Prisma.MovieWhereInput {
    const where: Prisma.MovieWhereInput = {
      ...
      // Library lists only streamable titles (at least one torrent).
      torrents: { some: {} },
    };
  ...
  buildOrderBy(
    ...
    switch (sortBy) {
      case "year": {
        ...
        return [{ year: direction }, { title: "asc" }, { id: "asc" }];
      }
      case "rating": {
        const direction = order === "asc" ? "asc" : "desc";
        return [
          { imdbRating: { sort: direction, nulls: "last" } },
          { title: "asc" },
          { id: "asc" },
        ];
      }
      ...
      default:
        // Fallback only — Popular browse uses TMDb encounter order in MoviesService.
        return [
          { popularity: "desc" },
          { imdbRating: { sort: "desc", nulls: "last" } },
          { id: "asc" },
  ...
  mapSortField(sortBy?: string): string | undefined {
    switch (sortBy) {
      ...
      case "seeds":
        return "seeds";
      default:
        // Match Popular grid — YTS browse without an explicit sort pulls by seeds.
        return "seeds";
    }
  }

...
export class MovieMapperService {
  ...
  toDetail(
    ...
    const torrents = movie.torrents
      ...
      // Postgres returns rows in an arbitrary order, so tie-break down to `id`:
      // an unstable order made the client pick a different release on every load.
      .sort(
        (a, b) =>
          (b.episodeLabel ?? "").localeCompare(a.episodeLabel ?? "") ||
          b.seeds - a.seeds ||
